...
//!
//! The vocabulary lives in `libs/wm_api` (`WmRequest::Preview` /
//! `Open`): a client names a path and, optionally, the app to open it
//! with; the WM resolves the association (`makepad_wm_api::viewer_for`), finds
...
pub struct ActivePreview {
    /// The tile that asked for this preview (files, say) — keeps key
    /// focus throughout and is the one told `PreviewShown`/`PreviewHidden`.
    pub requester: ClientId,
    /// The viewer app id (`makepad_wm_api::viewer_for`), also the `warm` key.
...
pub fn spawn_for_request(
    ...
    spawn_client(pool, spawner, &app, id, hub_port, None, None, &extra, false, lines)
}
...
mod tests {
    ...
    fn the_type_map_holds_one_warm_viewer_per_viewer_type() {
        ...
        cache.remember("image", 7);
        cache.remember("pdf", 8);
        assert_eq!(cache.warm_client("image", |_| true), Some(7));
        assert_eq!(cache.warm_client("pdf", |_| true), Some(8));
        // A type nobody has previewed yet has no warm client.
        assert_eq!(cache.warm_client("video", |_| true), None);
        // Respawning a type replaces its slot rather than accumulating.
        cache.remember("image", 11);
        assert_eq!(cache.warm_client("image", |_| true), Some(11));
    ...
    fn a_dead_viewer_clears_its_slot_at_the_next_request() {
        ...
        assert_eq!(cache.warm_client("video", dead), None);
        assert!(cache.warm.is_empty());
        assert!(cache.pending.is_empty());
        // ...and the cleared slot stays cleared for a live successor.
        cache.remember("video", 5);
        assert_eq!(cache.warm_client("video", |_| true), Some(5));
    }

    #[test]
    fn removing_a_client_forgets_it_everywhere() {
        let mut cache = PreviewCache::default();
        cache.remember("image", 3);
        cache.queue_pending(3, "/b.png".to_string());
        shown(&mut cache, 1, "image", 3, "/b.png");
        ...
        assert!(cache.active.is_none());
        assert_eq!(cache.warm_client("image", |_| true), Some(3));
    ...
    fn the_type_switch_and_the_requester_are_recognised() {
        let mut cache = PreviewCache::default();
        assert!(!cache.switching_type("image"), "nothing is up yet");
        shown(&mut cache, 1, "image", 3, "/b.png");
        assert!(!cache.switching_type("image"), "same type = a retarget");
        assert!(cache.switching_type("pdf"), "png -> pdf hides the float");
    ...
    fn a_close_all_takes_the_whole_cache_with_it() {
        ...
        assert_eq!(cache.warm_client("image", |_| true), None);
    }
}
