"""Exercise actual package storage authority, coupled policy, and partial readback."""
import os
...
class AppStorageBackendTest(unittest.TestCase):
    def test_observed_storage_policy_and_one_use_authority(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            sources+=list((ROOT/'vendor/octosense/settings/src/dev/makepad/octosense/appstorage').glob('*.java'))
            sources.append(ROOT/'tests/java/AppStorageBackendTest.java')
            for command in ([javac,'-d',tmp,*map(str,sources)],[java,'-cp',tmp,'AppStorageBackendTest']):
