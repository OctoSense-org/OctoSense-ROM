...
- **The unit is an ordinary bundle** under `system-apps/<name>/`: `manifest.json` and a `main.splash` program, plus small data and artwork. The build packs it (`octosense_app_hub::pack::pack_system_app`, from `apps/app-hub/build.rs`) and stamps the digest of exactly the packed files into the packed manifest. Large artwork is listed in `system-apps/<name>.assets.json`, compiled into the binary, and served from memory at bundle paths, so it is not packed, unpacked or hashed on every build.
