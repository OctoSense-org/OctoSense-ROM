...
impl PhoneSurface {
    ...
    pub fn draw_overlay(&mut self, cx: &mut Cx2d, state: &WmState, screen: Rect, backdrop: Option<GaussBlurSnapshot>, present: &mut dyn FnMut(&mut Cx2d, &Texture, Rect)) {
        ...
        // Mark the shell's actual swipe-start band, inside the safe area
        // above Android's navigation. Keep it after the hints are learned.
        // The keyboard excludes shell swipes, so it must not show this cue.
