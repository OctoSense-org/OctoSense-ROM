"""Exercise actual package battery authority, coupled policy, and partial readback."""
import os
...
class AppBatteryBackendTest(unittest.TestCase):
    def test_observed_battery_policy_and_one_use_authority(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            sources+=list((ROOT/'vendor/octosense/settings/src/dev/makepad/octosense/battery').glob('*.java'))
            sources.append(ROOT/'tests/java/AppBatteryBackendTest.java')
            for command in ([javac,'-d',tmp,*map(str,sources)],[java,'-cp',tmp,'AppBatteryBackendTest']):
