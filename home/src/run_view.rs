...
script_mod! {
    ...
    mod.widgets.MpRunView = set_type_default() do mod.widgets.MpRunViewBase {
        ...
        draw_app +: {
            ...
            fade: instance(1.0)
            // The corner clip (Sdf2d half-radius): 0 for a tile or a window,
            // whose rounding the desk masks through the captured surface;
            // the AI pane sets its glass card's radius so the body does not
            // overpaint the card's rounded corners.
            radius: instance(0.0)
            pixel: fn() {
                ...
                // Premultiplied, so one coverage multiply clips the whole
                // frame. The ramp is one DEVICE pixel wide (the SDF is in
                // logical px), or the outermost device row would sit
                // three-quarters covered at dpi 2: a hairline all round.
                let sdf = Sdf2d.viewport(self.pos * self.rect_size)
                sdf.box(0.0, 0.0, self.rect_size.x, self.rect_size.y, self.radius)
...
pub struct MpRunView {
    ...
    // The startup backdrop stops drawing once the child presents a frame.
    // Widget queries must follow the live surface, not that stale backdrop.
    #[area]
    #[rust]
    area: Area,
    ...
    #[rust(1.0f32)]
    fade: f32,
    /// The corner clip (Sdf2d half-radius) the host sets; 0 = square.
    #[rust]
    corner_radius: f32,
...
impl MpRunView {
    ...
    /// The corner clip, as an Sdf2d half-radius (half the visual radius);
    /// 0 leaves the frame square. Read at the next draw.
...
impl Widget for MpRunView {
    fn draw_walk(&mut self, cx: &mut Cx2d, scope: &mut Scope, walk: Walk) -> DrawStep {
        ...
        if self.present_ok_count == 0 {
            self.draw_bg.draw_vars.set_uniform(cx, live_id!(radius), &[self.corner_radius]);
...
impl crate::tile::TileHost for MpRunView {
    ...
    fn set_corner_radius(&mut self, radius: f32) {
        MpRunView::set_corner_radius(self, radius)
    }
}
